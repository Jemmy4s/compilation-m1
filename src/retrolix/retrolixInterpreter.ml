...
let print_runtime runtime =
  let idmap m =
    String.concat "," (List.map (fun (Id s, v) ->
      Printf.sprintf "%s = %s" s (print_data v)
    ) (IdMap.bindings m))
  in
  let ridmap m =
  ...
  and op runtime o vs =
    match o, vs with
      ...
      | BlockCreate, [ DInt size; init ] ->
        DLocation (Memory.allocate runtime.memory (Int32.to_int size) init)
      | BlockSet, [ DLocation location; DInt i; v ] ->
        let block = Memory.dereference runtime.memory location in
        Memory.write block (Int32.to_int i) v;
        DUnit
      | BlockGet, [ DLocation location; DInt i ] ->
        let block = Memory.dereference runtime.memory location in
        Memory.read block (Int32.to_int i)
      | Bool c, vs ->
...
let print_observable runtime obs =
  String.concat "\n" (List.map (fun (Id k, v) ->
    Printf.sprintf "%s = %s" k (print_data v)
