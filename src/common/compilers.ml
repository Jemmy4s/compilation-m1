...
let register source target m =
  compilers := (source, (target, m)) :: !compilers
...
let find compilers source target using = List.(ExtStd.List.Monad.(
  ...
  in
  let ps = run (search [] source target) in
  (* iter (fun p -> Printf.eprintf "%s\n" (String.concat " " (map fst p))) ps; *)
  ps
  |> filter (fun p -> for_all (fun u -> exists (fun (l, _) -> l = u) p) using)
  |> map (map snd)
))

let get ?(using=[]) source target =
  match find compilers source target using with
    | [] ->
      Error.global_error
        "during compilation"
