...
and binding (x, ty) =
  group (identifier x ++ group (string ":" ++ typ ty))

and typ = function
  | TyIdentifier x ->
    type_identifier x
  | TyTuple ts ->
    group (parens (separate_map (string " * ") typ ts))
  | TyArrow (ity, oty) ->
    group (group (parens_at_left_arrow ity (typ ity))
           ++ group (string "->" ++ typ oty))
...
and expression = function
  ...
  | Fun (x, e) ->
    nest 2 (group (
      group (string "{" ++ typed_identifier (Position.value x) ++ string "->")
      ++ expression' e ++ string "}")
    )
...
and branch (Branch (p, e)) =
  group (group (string "|" ++ pattern' p ++ string "->") ++ group (nest 2 (expression' e)))
