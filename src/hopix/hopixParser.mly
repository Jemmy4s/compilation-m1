%{
  ...
  let fresh_name =
    let r = ref 0 in
    fun () -> incr r; Id ("__force_type__" ^ string_of_int !r)

  let locate = Position.with_pos

  let force_type pos e = function
    | None ->
      e
    | Some ty ->
      let x = fresh_name () in
      locate pos (
        Apply (locate pos (Fun (locate pos (x, Some ty), locate pos (Variable x))),
               e)
      )
  ...
  let recfuns fs =
    let recfun (f, xs, out_ty, e) =
      ...
      let xs =
        List.map (fun (x, ty) -> Position.with_pos pos (x, ty)) xs
      in
      let e = force_type pos e out_ty in
      (Position.(map (fun f -> (f, None)) f), lambda xs e)
    in
...
tbinding: LPAREN x=identifier ty=typ_annotation RPAREN
{
  (x, ty)
}
| x=identifier
...
binding: LPAREN b=located(rbinding) RPAREN
{
  b
}
| x=located(identifier)
{
  Position.map (fun x -> (x, None)) x
}

rbinding: x=identifier COLON ty=typ
{
  (x, Some ty)
}
| x=identifier
